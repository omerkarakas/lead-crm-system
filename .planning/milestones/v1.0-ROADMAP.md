...
- Phase 4.1: Lead Capture & Pipeline Automation (inserted after Phase 4 for critical missing features)
- Phase 4.2: Proposal Management (inserted after Phase 4.1 for complete sales workflow)

**Key Decisions:**

- Next.js 14 + TypeScript + Tailwind CSS (App Router, Server Components, modern DX)
- PocketBase as backend (self-contained, Go-based, easy deployment)
- Cookie-based auth persistence (better UX, survives browser refresh)
- Zustand for state management (lightweight, simple API, no boilerplate)
- Turkish language UI (target audience language preference)
- Role-based access control (RBAC) with 3 roles (Admin, Sales, Marketing)
- 80-point quality threshold for lead qualification
- Fire-and-forget messaging pattern (log errors but don't throw to prevent cascade failures)

**Issues Resolved:**

- Auth redirect issue fixed with server-side token validation
- PocketBase API rules configuration (createdBy nonempty property removed)
- UTM fields, message field, email optional migrations applied
- WhatsApp webhook typeWebhook filtering (status vs message webhooks)
- Question data manual fetch (expand not working, implemented batch fetch)
- QA Answers section removed from lead detail (redundant with score modal)
...
- Welcome message stored in localStorage (consider moving to PocketBase config)
